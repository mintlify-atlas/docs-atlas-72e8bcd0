---
title: "Inventory Management for SansiStore Operators"
...
<CardGroup cols={2}>
  ...
  <Card title="Pedidos con fallos" icon="package-x" href="https://sansistore-umss.vercel.app/inventory/incidents">
    Lists orders whose status is `NO ENTREGADO` or `CANCELADO`. Operators can restore reserved stock back to available in a single Firestore transaction, which also records an ENTRADA movement and sets `stockRestored: true` on the order.
  </Card>
...
<Note>
  `stockAvailable` is always derived as `stockTotal − stockReserved`. When `stockAvailable` drops at or below `minStock`, the dashboard highlights the product row and a `StockAlertToast` is shown to the operator. This threshold is configurable per product.
</Note>
